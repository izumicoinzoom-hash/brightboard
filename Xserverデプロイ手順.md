...
- **ドメイン直下**（例: `https://あなたのドメイン/`）に出す  
  → `FTP_REMOTE_DIR` は **`public_html`**
- **サブフォルダ**（例: `https://あなたのドメイン/kiyota/`）に出す  
  → `FTP_REMOTE_DIR` は **`public_html/kiyota`** など（フォルダ名は任意）
...
| `FTP_REMOTE_DIR` | `public_html` または `public_html/kiyota` など | アップロード先のリモートパス（[2. 公開する場所](#2-公開する場所を決める)で決めた値） |
